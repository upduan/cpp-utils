...
namespace utils::Texts {
    ...
    std::vector<std::wstring> const split(std::wstring const& data, std::wstring const& delimitation) {
        ...
        return std::vector<std::wstring>(std::wsregex_token_iterator(data.begin(), data.end(), re, -1), std::wsregex_token_iterator());
    ...
    std::vector<std::string> const split(std::string const& data, std::string const& delimitation) {
        ...
        return std::vector<std::string>(std::sregex_token_iterator(data.begin(), data.end(), re, -1), std::sregex_token_iterator());
    ...
    std::string const FormatDateTime(short year, short month, short date, short hour, short minute, short second) {
        ...
        // SYSTEMTIME tm;
        // GetLocalTime(&tm);
        // std::sprintf_s(dateTime, "%04d-%02d-%02d %02d:%02d:%02d", tm.wYear, tm.wMonth, tm.wDay, tm.wHour, tm.wMinute, tm.wSecond);
        std::sprintf_s(dateTime, "%04d-%02d-%02d %02d:%02d:%02d", year, month, date, hour, minute, second);
